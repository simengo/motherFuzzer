...
public class SenseTestMark3 extends SystemTest {
    @Override
    public void test(GameInfo gameInfo) {
        String map = "2\n2\n" +
            ".A\n" +
            "B.";
        String brain = "brain \"sample\" {\nturn left\nmove else 0\nmark 3\nmove else 0\nturn right\nsense left marker 3 else 1\nturn right\njump 0\n}";
       ...
       if(!(dir.equals(ant.getDirection()))){
           fail("mark 3 was left so ant did turn");
       }
       ...
       ant = world.getAnt(0);
        if(dir.equals(ant.getDirection())) {
            fail("mark 3 was not right so ant did not turn");
        }

        brain = "brain \"sample\" {\nturn left\nmove else 0\nmark 3\nmove else 0\nturn right\nsense ahead marker 3 else 1\nturn right\njump 0\n}";
        world =  gameInfo.simulate(34,42,map,brain,brain0);
        ant = world.getAnt(0);
        if(dir.equals(ant.getDirection())) {
            fail("mark 3 was not ahead so ant did not turn");
        }
        ...
        ant = world.getAnt(0);
        if(dir.equals(ant.getDirection())) {
            fail("here  not mark 3  so ant did not turn");
        }
